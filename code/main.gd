...
var _pause_btn_label:   Node  = null
...
var time_limit_seconds  := 60
var _turn_timer         := 60.0
var _prev_timer_player  := -1
...
var new_rule_enabled        := false
var _white_last_move_active := false
...
func _on_game_started(time_limit: bool, time_secs: int, new_rule: bool) -> void:
	time_limit_enabled = time_limit
	time_limit_seconds = time_secs
	new_rule_enabled   = new_rule
	_start_game_impl()

func _on_ai_game_started(difficulty: int, human_player: int, time_limit: bool, time_secs: int, new_rule: bool) -> void:
	is_ai_mode         = true
...
func _process(delta: float) -> void:
	if not time_limit_enabled or in_menu or game_over or is_animating: return
	if _pause_menu != null and _pause_menu.is_paused: return
	var human_turn := not is_ai_mode or current_player != ai_player
	if current_player != _prev_timer_player:
		_prev_timer_player = current_player
		if human_turn:
			_turn_timer = float(time_limit_seconds)
			_update_timer_ui()
		return
	...
	if _turn_timer <= 0.0:
		_turn_timer        = float(time_limit_seconds)
		_prev_timer_player = -1
		var valid := []
		for c in range(BOARD_SIZE):
			if board[c][0] == 0:
				valid.append(c)
		if not valid.is_empty():
			drop_piece(valid[randi() % valid.size()])


func _update_timer_ui() -> void:
	if not is_instance_valid(_timer_label): return
	var secs := ceili(_turn_timer)
	_timer_label.text = str(secs)
	var limit := float(time_limit_seconds)
	if _turn_timer <= limit * 0.17:
		_timer_label.add_theme_color_override("font_color", Color(1.0, 0.3, 0.3))
	elif _turn_timer <= limit * 0.34:
		_timer_label.add_theme_color_override("font_color", Color(1.0, 0.75, 0.2))
	else:
...
func _notification(what: int) -> void:
	if what == NOTIFICATION_WM_GO_BACK_REQUEST:
		if not in_menu and _pause_menu != null:
			_pause_menu.toggle()
	...
	for row in range(BOARD_SIZE - 1, -1, -1):
		if board[col][row] == 0:
			undo_stack.push_back({  # 落子前存快照
				...
				"turn_before": turn_in_round,
				"was_white_last_move": _white_last_move_active
			})
...
func start_rotation_sequence(save_undo = true):  # save_undo=false 供 redo 呼叫時使用
	...
	_rotation_tween = create_tween().set_parallel(true)
	var target_rot = board_container.rotation + deg_to_rad(90)
	_rotation_tween.tween_property(board_container, "rotation", target_rot, 0.8).set_trans(Tween.TRANS_QUINT).set_ease(Tween.EASE_IN_OUT)
	for child in piece_container.get_children():
		var old_pos = child.position
		var new_pos = Vector2(-old_pos.y, old_pos.x)
		_rotation_tween.tween_property(child, "position", new_pos, 0.8).set_trans(Tween.TRANS_QUINT).set_ease(Tween.EASE_IN_OUT)
...
func apply_gravity_and_animate():
	...
	if not drop_pieces.is_empty():
		...
		for item in drop_pieces:
			if is_instance_valid(item["piece"]):
...
func check_winner_logic(after_rotation: bool) -> bool:
	...
	if after_rotation:
		...
		if _white_last_move_active:
			# 白方最後一子落下，決定結果：連四就白勝，否則黑勝
			_white_last_move_active = false
			if w_reached:
				game_over = true
				for child in piece_container.get_children():
					child.stop_glow()
...
func show_result(text: String):
	...
	_show_result_subtitle()


func _start_white_last_move() -> void:
	_white_last_move_active = true
	current_player = 2
	highlight_winning_pieces(1)
	_schedule_ai_move()


func _show_result_subtitle() -> void:
	if is_instance_valid(_result_subtitle):
		_result_subtitle.queue_free()
	var vp  := get_viewport_rect().size
	var lbl := Label.new()
	lbl.text = "TAP TO RESTART"
	lbl.add_theme_font_override("font", preload("res://fonts/PressStart2P-Regular.ttf"))
	lbl.add_theme_font_size_override("font_size", int(vp.y * 0.028))
	lbl.add_theme_color_override("font_color", Color.GRAY)
	lbl.size                 = Vector2(vp.x, vp.y * 0.06)
	lbl.position             = Vector2(0.0, vp.y * 0.5 + vp.y * 0.13)
	lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	lbl.vertical_alignment   = VERTICAL_ALIGNMENT_CENTER
	lbl.mouse_filter         = Control.MOUSE_FILTER_IGNORE
	lbl.modulate.a           = 0.0
	$ResultLabel.add_child(lbl)
	_result_subtitle = lbl

	var tween := lbl.create_tween()
	tween.tween_interval(1.0)
	tween.tween_property(lbl, "modulate:a", 1.0, 0.5).set_trans(Tween.TRANS_SINE)
	tween.tween_callback(func():
		var loop := lbl.create_tween().set_loops()
		loop.tween_property(lbl, "modulate:a", 0.0, 0.6).set_trans(Tween.TRANS_SINE)
		loop.tween_property(lbl, "modulate:a", 1.0, 0.6).set_trans(Tween.TRANS_SINE)
	)
...
func return_to_menu() -> void:
	...
	if is_instance_valid(_timer_label):     _timer_label.queue_free()
	_undo_label        = null
	_redo_label        = null
...
func _clear_game_over():  # undo 時如果遊戲已結束或在白方最後一子狀態，清除狀態
	if not game_over and not _white_last_move_active:
		return
	game_over = false
	_white_last_move_active = false
	result_label.visible = false
...
func perform_undo():
	...
	if is_animating:  # 動畫中（非 AI 思考）則不執行
		return
	...
	if entry["type"] == "drop":
		...
		_update_undo_redo_ui()
		if entry.get("was_white_last_move", false):
			_white_last_move_active = true
			highlight_winning_pieces(1)
		is_animating = false
...
func perform_redo():
	...
	if entry["type"] == "drop":
		...
		var anim = play_drop_animation(col, row, player)
...
func _create_game_ui() -> void:
	...
	var icon := preload("res://code/pause_icon.gd").new()
	icon.z_index = 1
	add_child(icon)
	icon.position = Vector2(pause_x + btn_w / 2.0, pause_y + btn_h / 2.0)
	icon.setup(btn_h)
	_pause_btn_label = icon
	_pause_btn_rect = Rect2(Vector2(pause_x, pause_y), Vector2(btn_w, btn_h))

	if time_limit_enabled:
		_timer_label = Label.new()
		_timer_label.add_theme_font_override("font", font)
		_timer_label.add_theme_font_size_override("font_size", sz)
		_timer_label.add_theme_color_override("font_color", Color.WHITE)
		...
		_timer_label.size                 = Vector2(btn_w, btn_h)
		_timer_label.position             = Vector2(CENTER_POS.x - btn_w * 0.5, pause_y - btn_h)
...
func _schedule_ai_move() -> void:
	...
	_ai_thread.start(func(): _ai_result = _ai.best_move(board_copy, ai_player, turn_in_round, ai_difficulty, new_rule_enabled))
	while _ai_thread != null and _ai_thread.is_alive():
		await get_tree().process_frame
	if _ai_thread == null:
		return
	_ai_thread.wait_to_finish()
	...
	var col := _ai_result
	is_animating = false
	while _pause_menu != null and _pause_menu.is_paused:
		await get_tree().process_frame
...
func _create_bgm_player() -> void:
	var stream := load("res://music/acoustic-background-music-loop-391027.mp3") as AudioStreamMP3
	...
	_bgm_player.play()
	_bgm_duck_tween = _bgm_player.create_tween()
	_bgm_duck_tween.set_pause_mode(Tween.TWEEN_PAUSE_PROCESS)
	_bgm_duck_tween.tween_property(_bgm_player, "volume_db", 0.0, 2.0).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
...
func duck_bgm() -> void:
	...
	_bgm_duck_tween.set_pause_mode(Tween.TWEEN_PAUSE_PROCESS)
	_bgm_duck_tween.tween_property(_bgm_player, "volume_db", linear_to_db(0.3), 0.5)
...
func unduck_bgm() -> void:
	...
	_bgm_duck_tween = create_tween()
	_bgm_duck_tween.set_pause_mode(Tween.TWEEN_PAUSE_PROCESS)
	_bgm_duck_tween.tween_property(_bgm_player, "volume_db", 0.0, 0.5)
